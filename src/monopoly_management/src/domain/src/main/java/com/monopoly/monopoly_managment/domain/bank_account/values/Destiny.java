package com.monopoly.monopoly_managment.domain.bank_account.values;

import com.monopoly.shared.domain.generic.IValueObject;
import com.monopoly.shared.domain.utils.Validator;

public class Destiny implements IValueObject {
    private final String value;

    private Destiny(final String value) {
        this.value = value;
        validate();
    }

    public static Destiny of(final String value) {
        return new Destiny(value);
    }

    public String getValue() {
        return value;
    }

    @Override
    public void validate() {
        Validator.validateNull(this.value, "Destiny value");
    }
}
